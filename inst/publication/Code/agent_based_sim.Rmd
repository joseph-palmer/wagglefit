...
The code for the honeybee foraging simulation is written in python and is contained in the `Simulation` directory (specifically `Simulation/Code`). The results of the simulation (as output csv files) are stored in `Results/`. Generating the results can be done by running the script `run.py` in `Simulation/Code`. Once you have ran the simulation you can run through this Rmd doc to generate the figures. To run the python code you will need python 3 installed along with the required packages.
...
# load data
recruit <- read.csv(
  "Simulation/Results/recruit_distribution.csv",
  header = FALSE
)
scout <- read.csv(
  "Simulation/Results/scout_distribution.csv",
  header = FALSE
)
...
combplot <- ggplot() +
  geom_point(
    data = scout_cdf, aes(x = sd, y = log(prob), color = "blue"), size = 0.2
  ) +
  geom_point(
    data = recruit_cdf, aes(x = sd, y = log(prob), color = "red"), size = 0.2
  ) +
  labs(
    x = "Simulated foraging distance (km)",
    y = "ln Inverse cummulative frequency"
  ) +
  scale_color_manual(labels = c("Scout", "Recruit"), values = c("blue", "red")) +
  theme(legend.position="bottom")

ggsave(
  plot = combplot,
  filename = "Simulation/Results/combined_fit.png",
  width = 70.1,
  height = 81.79,
  units = "mm",
  dpi = 400
...
```
From this analysis we observe that the distribution of foraging distances reported by scouts is better described by an exponential than the minimum of an exponential, whereas the converse is true for foraging distances reported by recruits. These results support our hypothesis for how scout and recruit reported foraging distances occur through different processes and produce different patterns.
