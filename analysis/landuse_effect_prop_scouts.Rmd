...
# Proportion of landuse in Urban and Agri-rural

```{r land-use-coverage, message = FALSE, warning = FALSE, error = TRUE}
rural_total_area <- rural_lu[, 3:ncol(rural_lu)] %>%
  colSums()
rural_total_area <- tibble(
  landuse_type = names(rural_total_area), area = rural_total_area
)
rural_total_area$area_pct <- (
  rural_total_area$area / sum(rural_total_area$area)
) * 100

rural_total_area <- rural_total_area %>%
  select(landuse_type, area_pct) %>%
  rename(
    Landuse = landuse_type, "% coverage" = area_pct
  ) %>%
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", rural_total_area$landuse_type)
    ),
    Landuse = ifelse(Landuse == "OSR", "Oilseed Rape", Landuse)
  ) %>%
  rename(
    "Land-use" = Landuse
  )
...
urban_total_area <- urban_lu[, 3:ncol(urban_lu)] %>%
  colSums()
urban_total_area <- tibble(
  landuse_type = names(urban_total_area), area = urban_total_area
)
urban_total_area$area_pct <- (
  urban_total_area$area / sum(urban_total_area$area)
) * 100

urban_total_area <- urban_total_area %>%
  select(landuse_type, area_pct) %>%
  rename(
    Landuse = landuse_type, "% coverage" = area_pct
  ) %>%
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", urban_total_area$landuse_type)
    )
  ) %>%
  rename(
    "Land-use" = Landuse
...
save(urban_total_area, rural_total_area, file = "results/area_coverage.RData")
```
...
# make a scree plot
ggplot(
  data.frame(PC = seq(1, 9), varpercent = varpercent),
  aes(x = PC, y = varpercent)
) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = 1 / ncol(urban.pcadata) * 100, col = "red") +
...
# Figures, results write up and main discussion points

input figure stuff here.

```{r, manuscript-figures, message = FALSE, warning = FALSE}

# load loadings data from the jk analysis
loading_path <- "results/pls-loadings.RData"
if (file.exists(loading_path)) {
  load(loading_path)
} else {
  stop(
    paste(
      "File", loading_path, "not found.",
      "Please run the notebook `landuse_effect_jackknife.Rmd` to create it."
    )
  )
}
...
ar_fit <- ggplot(
  ...
  xlab("PC 1") +
  ylab("Estimated waggle dance use")
...
for (i in seq_len(3)) {
  ...
  site_plots[[ar_marked[i, "site"]]] <- ggplot() +
    ...
    scale_fill_manual(
      values = c(
        "arable" = "#018B27", "built-up area" = "#ED6778",
        "Non-agricultural unmanaged green space" = "#BEC330",
        "water" = "#536CAB", "pasture" = "#BBBBBB", "woodland" = "#BBBBBB",
...
# make loadings plot
argi.rural.loadings <- rural_jk_loadings %>%
  rename(
  ...
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", rural_jk_loadings$landuse_type)
    ),
    significant = ifelse(
      Correlation > rural_jk_loadings$cutoff,
      Landuse,
      ifelse(
        Correlation < -rural_jk_loadings$cutoff,
        Landuse,
...
cutoff <- unique(argi.rural.loadings$cutoff)[[1]]

rural_loading_text <- tibble(
  landuse_type = unique(argi.rural.loadings$Landuse), result = 0
)

loading_plot <- ggplot(
  data = argi.rural.loadings,
  aes(
    x = Landuse, y = Correlation
  )
) +
  geom_boxplot(aes(fill = Landuse), lwd = 0.1) +
  geom_hline(
  ...
  geom_text(
    ...
    hjust = 0.5,
    vjust = 0.5,
    nudge_y = c(-0., -0.), nudge_x = c(0.5, 0.5), size = rel(textsize)
  ) +
  annotate(
    "rect",
    ymin = -cutoff,
...
u_fit <- ggplot(
  ...
  theme(legend.position = "none", text = element_text(size = rel(textsize - 2))) +
  xlab("PC 1") +
  ylab("Estimated waggel dance use")
  ...
  site_plots[[u_marked[i, "site"]]] <- ggplot() +
    ...
    scale_fill_manual(
      values = c(
        ...
        "sparse residential" = "#ED6778", "amenity grassland" = "#BBBBBB",
        "water" = "#BBBBBB", "parks allotments cemeteries" = "#018B27",
        "Railway" = "#BEC330", "woodland" = "#BBBBBB"
      )
    )
}
...
# make loadings plot
urban.loadings <- urban_jk_loadings %>%
  rename(
    Landuse = landuse_type,
    Correlation = loadings
  ) %>%
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", urban_jk_loadings$landuse_type)
    ),
    significant = ifelse(
      Correlation > urban_jk_loadings$cutoff,
      Landuse,
      ifelse(
        Correlation < -urban_jk_loadings$cutoff,
        Landuse,
...
cutoff <- unique(urban.loadings$cutoff)[[1]]

urban_loading_text <- tibble(
  landuse_type = unique(urban.loadings$Landuse), result = 0
)
...
loading_plot <- ggplot(
  ...
  scale_fill_manual(
    values = c(
      "Continuous Central" = "#BBBBBB", "Dense Residential" = "#BBBBBB",
      "Sparse Residential" = "#ED6778", "Railway" = "#BEC330",
      "Parks Allotments Cemeteries" = "#018B27",
