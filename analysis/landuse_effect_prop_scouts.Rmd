...
for (i in seq_len(3)) {
  ...
  site_shape_data[[ar_marked[i, "site"]]] <- df
...
loading_plot <- ggplot(argi.rural.loadings, aes(x = Landuse, y = Correlation)) +
  ...
  scale_colour_manual(
    values = c(
      "arable" = "red3", "built.up.area" = "orange3",
      "Non.agricultural.unmanaged.green.space" = "seagreen3",
      "water" = "cornflowerblue", other = "grey"
    )
  ) +
  ylim(-0.6, 0.6)
...
mfitpls <- plot_grid(
  ar_fit + theme(plot.margin = unit(c(0., 0., 1., 0.), "cm")),
  loading_plot,
  ncol = 1,
  align = "v",
  labels = c(
...
for (i in seq_len(3)) {
  ...
  site_shape_data[[u_marked[i, "site"]]] <- df
...
loading_plot <- ggplot(urban.loadings, aes(x = Landuse, y = Correlation)) +
  ...
  scale_colour_manual(
    values = c(
      "continuous..central" = "red3", "dense.residential" = "orange3",
      "sparse.residential" = "purple3", "amenity.grassland" = "seagreen3",
      "water" = "cornflowerblue", other = "grey"
    )
  ) +
