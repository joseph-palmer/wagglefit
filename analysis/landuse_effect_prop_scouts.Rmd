...
argi.rural.loadings <- rural_jk_loadings %>%
  ...
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", rural_jk_loadings$Landuse)
    ),
    significant = ifelse(
...
rural_loading_text <- tibble(
  landuse_type = unique(argi.rural.loadings$Loadings), result = 0
)
loading_plot <- ggplot(data = argi.rural.loadings, aes(
  x = Landuse, y = Correlation, fill = Landuse
)) +
  geom_boxplot() +
  geom_hline(
...
urban.loadings <- urban_jk_loadings %>%
  ...
  mutate(
    Landuse = tools::toTitleCase(
      gsub("\\.", " ", urban_jk_loadings$Landuse)
    ),
    significant = ifelse(
...
loading_plot <- ggplot(data = urban.loadings, aes(
