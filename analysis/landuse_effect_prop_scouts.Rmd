...
ar_fit <- ggplot(
  ...
  geom_text(
    ...
    nudge_x = c(0, 0), nudge_y = c(-0.05, 0.05), size = rel(3)
  ) +
  theme_classic() +
...
for (i in seq_len(3)) {
  ...
  site_plots[[ar_marked[i, "site"]]] <- ggplot() +
    ...
    scale_fill_manual(
      values = c(
        "arable" = "#EE6677", "built-up area" = "#CCBB44",
...
loading_plot <- ggplot(argi.rural.loadings, aes(x = Landuse, y = Correlation)) +
  geom_point(aes(colour = significant), size = rel(3)) +
  geom_segment(aes(
  ...
  geom_text(
    ...
    hjust = 0.25,
    nudge_y = c(0, 0), nudge_x = c(0.5, 0.5), size = rel(3)
  ) +
  geom_hline(
...
sitelumaps <- plot_grid(
  site_plots[[3]] + theme(legend.position = "none"),
  site_plots[[2]] + theme(legend.position = "none"),
  site_plots[[1]] + theme(legend.position = "none"),
  ncol = 1,
  align = "v",
  labels = c(
    names(site_plots)[3], names(site_plots)[2], names(site_plots)[1]
  )
)

# now the model fit and pls results in a plot of two
mfitpls <- plot_grid(
  ar_fit + theme(plot.margin = unit(c(0., 0., 0.2, 0.), "cm")),
  loading_plot,
  ncol = 1,
  align = "v",
  labels = c(
...
sitelumaps <- plot_grid(
  site_plots[[1]] + theme(
    legend.position = "none",
    plot.margin = unit(c(0.1, 0.2, -1.2, 0.2), "cm")
  ),
  site_plots[[2]] + theme(
    legend.position = "none",
    plot.margin = unit(c(0.1, 0.2, -1.2, 0.2), "cm")
...
mfitpls_top <- plot_grid(
  sitelumaps + theme(plot.margin = unit(c(0.2, 0.2, 1.2, 0.2), "cm")),
  ar_fit + theme(plot.margin = unit(c(0.2, 0.2, 0.2, 0.2), "cm")),
  ncol = 1,
  align = "v",
  rel_heights = c(.5, 1),
  labels = c(
...
for (i in seq_len(3)) {
  ...
  site_plots[[u_marked[i, "site"]]] <- ggplot() +
    ...
    scale_fill_manual(
      values = c(
        "continuous central" = "#EE6677", "dense residential" = "#AA3377",
        "sparse residential" = "#CCBB44", "amenity grassland" = "#228833",
        "water" = "#4477AA", "parks allotments cemeteries" = "#BBBBBB",
        "Railway" = "#BBBBBB", "woodland" = "#BBBBBB"
      )
    )
}
...
loading_plot <- ggplot(urban.loadings, aes(x = Landuse, y = Correlation)) +
  ...
  scale_y_continuous(breaks = seq(-.9, .9, by = 0.4), limits = c(-1, 1))
...
# now the model fit and pls results in a plot of two
mfitpls <- plot_grid(
  u_fit + theme(plot.margin = unit(c(0., 0., 1., 0.), "cm")),
  loading_plot,
  ncol = 1,
  align = "v",
  labels = c(
